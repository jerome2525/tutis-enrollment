...
(function($){
	$.fn.selectstyle = function(option){
		...
		this.each(function(){
			...
			$this.find('option').each(function (e) {
				var $this_a = $(this),
					...
					data_active = $this_a.attr('data-programid'),
					text = $this_a.html();
				if(val == null){
					val = text;
				}
				html_op += '<li data-programid="'+data_active+'" data-title="'+text+'" value="'+val+'"';
			...
			'<div class="selectstyle ss_dib '+setting.theme+'" style="width:'+parseInt(setting.width)+'px;">'+
				...
				'</div>';
			if(search == "true"){
				html += '<ul id="select_style_ul" sid="'+id+'" class="ss_ulsearch" style="max-height:'+setting.height+'px;width:'+(parseInt(setting.width) + 20)+'px;"><div class="search" id="ss_search"><input type="text" placeholder="Search" class="focus-visible" autofocus></div><ul style="max-height:'+(parseInt(setting.height) - 53)+'px;width:'+(parseInt(setting.width) + 20)+'px;" class="ss_ul">'+html_op+'</ul></ul>';
			}
			else{
		...
		$("body").delegate( "div#ss_search input", "keyup", function(e) {
			...
			var val_cap = $(this).val().toUpperCase();
			$('#nosearch').remove();
			var prog_selected = $( "#program1 option:selected" ).val();
			$(this).parent().parent().find('li').each(function(index, el) {
				if( prog_selected ) {
					var prog_id = $(el).attr('data-programid');
					if( prog_selected == prog_id ) {
						if($(el).text().toUpperCase().indexOf(val_cap) > -1){
							$(el).show();
							flag=true;
